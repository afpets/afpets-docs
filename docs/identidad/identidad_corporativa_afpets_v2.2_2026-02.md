...
- 🧪 **Activación QR (definición operativa):** el QR se activa por quien lo escanea (puede ser el canal en punto de venta o el dueño al llevárselo). El proceso es el mismo: escanear → iniciar registro → completar datos básicos de mascota (**sin requerir pago al inicio**).
  ...
  - **Control de cobro:** el checkout/link de pago es de **AFPets** (no del canal), para mantener control de caja.
  - **Pendiente 🧪:** resolver recuperación del costo físico del QR (tag fee / primer mes / stock por canal / consignación).
